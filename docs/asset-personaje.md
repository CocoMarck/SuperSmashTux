...
- `lying_down`: Tumbado en el piso. Lleva a estado al person.
- `front_collision`: Colision de frente en cualquier lado.
- `back_collision`: Colision de espaldas en cualquier lado.
- `turn_around`: Dar vuelta. (Saltando no puede dar vuelta).

#### Fighter
- `neutral_attack`: Cendo no se mueve y esta parado.
- `crouch_attack`: Cuando esta agachado y sin moverse.
- `dash_attack`: Corriendo y atacar.
- `walk_attack`: Caminando y attack.
- `up_attack`: Sin moverse, y precionar ataque arriba.
- `hard_side_attack`: Cuando se preciona al mismo tiempo ataque y der o izq.
- `hard_up_attack`: Sin moverse, y precionar al mismo tiempo ataque y arriba.
- `hard_down_attack`: Sin moverse, y precionar al mismo tiempo ataque y abajo.
- `pick`: Agarrar item.
- `grab`: Agarrar a la gente.
- `protection`: Animación de defensa. Protección.
- `roll_forward`: Vuelta estilo parkur.
- `roll_back`: Vuelta estilo parkur.

#### PowerFighter
- `power_neutral`: Proyectil, rafaga de energia, o carga de ataque.
- `power_dash`: Correr y poder hacia un lado.
...
- `jump`: Saltando.
- `fall`: Cayendo.
- `air_damage`: Daño recibido en el aire.

#### Fighter
- `neutral_air_attack`: En el aire, sin moverse y atacar.
- `back_air_attack`: En el aire moviendose a de espaldas y atacar.
- `forward_air_attack`: En el aire moviendose a un lado y atacar.
- `down_air_attack`: En el aire y atacar precionando abajo. Ya sea moviendose o no.
- `up_air_attack`: En el aire y atacar precionando arriba. Ya sea moviendose o no.

#### PowerFighter
- `second_jump`: Segundo salto.
...
- `power_air_move`: Lo mismo que power dash. Pero animacion en el aire.